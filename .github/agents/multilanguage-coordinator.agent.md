...
Rust, TypeScript, TSX, JavaScript, JSX, Python, C#, Go, Java, C, and C++ syntax retrieval are all fully supported, each with the full acceptance bar below (parser/chunk/update/delete/restart/decoy-ranking/LSP-status). Rust, C#, TypeScript/TSX/JavaScript/JSX, Python, Go, and Java additionally have optional real-language-server navigation (rust-analyzer, csharp-ls, typescript-language-server, pyright, gopls, jdtls); C and C++ have no language-server adapter yet. Add any future language sequentially through the language-adapter contract. Add its optional language-server adapter only after syntax, persistence, and retrieval acceptance passes -- then record that status against acceptance.md's LSP checklist item, honestly, for whichever state is true.
