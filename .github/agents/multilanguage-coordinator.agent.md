...
Rust, TypeScript, TSX, JavaScript, JSX, Python, and C# syntax retrieval are already supported, each with the full acceptance bar below (parser/chunk/update/delete/restart/decoy-ranking). Rust, C#, TypeScript/TSX/JavaScript/JSX, and Python additionally have optional real-language-server navigation (rust-analyzer, csharp-ls, typescript-language-server, pyright). Go has syntax chunking and retrieval (parser/chunk acceptance only, added without the incremental-reuse, deletion-reconciliation, restart-reuse, and production-vs-decoy tests the other languages have -- do not describe Go as fully accepted until that gap is closed) and no language-server adapter yet. Add any future language sequentially through the language-adapter contract. Add its optional language-server adapter only after syntax, persistence, and retrieval acceptance passes.
