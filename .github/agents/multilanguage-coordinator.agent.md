...
Rust, TypeScript, TSX, JavaScript, JSX, Python, C#, and Go syntax retrieval are all fully supported, each with the full acceptance bar below (parser/chunk/update/delete/restart/decoy-ranking). Rust, C#, TypeScript/TSX/JavaScript/JSX, and Python additionally have optional real-language-server navigation (rust-analyzer, csharp-ls, typescript-language-server, pyright); Go has no language-server adapter yet. Add any future language sequentially through the language-adapter contract. Add its optional language-server adapter only after syntax, persistence, and retrieval acceptance passes.
