...
14. LSP navigation available via the language-server adapter contract, passing `$lci-lsp-adapter`'s own acceptance bar (real workspace symbol/definition/reference evidence, provider-isolation, and recovery).

Use mocks for deterministic automated coverage and the configured 8766/8767 services for one bounded live acceptance. Report those evidence classes separately. LSP evidence belongs to `$lci-lsp-adapter`; its acceptance bar is the source of truth for item 14's evidence, but the requirement itself is not optional or deferrable. A language-support claim is incomplete without it -- do not stop at syntax-only support, do not merge or announce a language as "added" before its navigation adapter passes, and do not treat "no language-server adapter yet" as an acceptable final state. If a real blocker makes navigation genuinely infeasible for a language (no LSP implementation exists anywhere for it, for example), say so explicitly to the user and get their agreement before treating item 14 as satisfied by anything less than passing evidence -- do not decide that unilaterally.
